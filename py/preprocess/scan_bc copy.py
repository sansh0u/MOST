...
def hmm_segment_entropy(entropy, n_states=2):

    """
    Gaussian HMM segmentation
    ...
    X = np.array(entropy).reshape(-1, 1)
...
# -------------------------------
    ...
    states, segments = hmm_segment_entropy(
    smooth_ent,
    n_states=2
    )
    ...
    for s, e, state in segments:
        ...
        label = "LOW" if state == 0 else "HIGH"
    ...
    left_entropy = smooth_ent[left_s:left_e]
